...
void PDBOutput::InitPartVec(pdb_setup::Atoms const& atoms)
{
 ...
 void PDBOutput::GetDataFromGPU()
 {
	
   cudaMemcpy(sysRef->calcEnergy.cordsx,sysRef->calcEnergy.Gpu_x,sizeof(double) * sysRef->calcEnergy.currentCoords.Count() ,	cudaMemcpyDeviceToHost);
   cudaMemcpy(sysRef->calcEnergy.cordsy,sysRef->calcEnergy.Gpu_y,sizeof(double) * sysRef->calcEnergy.currentCoords.Count() ,	cudaMemcpyDeviceToHost);
   cudaMemcpy(sysRef->calcEnergy.cordsz,sysRef->calcEnergy.Gpu_z,sizeof(double) * sysRef->calcEnergy.currentCoords.Count() ,	cudaMemcpyDeviceToHost);
   ...
   int ctr =0;
      ...
      while(tm != en)
      {
            ...
            for(uint i = 0; i <  sysRef->calcEnergy.mols.GetKind(*tm).NumAtoms(); ++i) 
            {
             	sysRef->coordinates.x[ sysRef->calcEnergy.mols.start[*tm] + i ] = sysRef->calcEnergy.cordsx[ctr ];
				sysRef->coordinates.y[ sysRef->calcEnergy.mols.start[*tm] + i ] = sysRef->calcEnergy.cordsy[ctr ];
				sysRef->coordinates.z[ sysRef->calcEnergy.mols.start[*tm] + i ] = sysRef->calcEnergy.cordsz[ctr ];
                ctr++;
            }
         ++tm;
      }
...
{   
  ...
  if(enableOutState)
   ...
   //NEW_RESTART_CODE
   if (((step + 1) % stepsRestPerOut == 0) && enableRestOut)//v1
   {
