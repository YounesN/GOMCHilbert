 ...
 public:
  ...
  void FillWithRandom(XYZArray & loc, const uint len, XYZ const& axis)//v1
		...
		const double subDraw, const double subPerc)
   {
     ...
     uint PickMol(uint & m, uint & mk, const uint b,
		const double subDraw, const double subPerc , uint & mf)//  
   {
      uint rejectState = mv::fail_state::NO_FAIL;
      uint mkTot = molLookRef.GetNumKind();
      double molDiv = subPerc/mkTot;
		      ...
		      double subDraw, const double movPerc)
   {
    ...
    uint PickMolAndBox(uint & m, uint &mk, uint &b, uint &mOff,
		      double subDraw, const double movPerc) //  
   {
      double boxDiv=0; 
      PickBox(b, subDraw, boxDiv, movPerc);
      return PickMol(m, mk, b, subDraw, boxDiv,mOff);
   }

	 uint PickMolAndBoxPair(uint &m, uint &mk,uint &mOff, uint & bSrc, uint & bDest,
			  double subDraw, const double movPerc)//  
   { 
      double boxDiv=0;
      PickBoxPair(bSrc, bDest, boxDiv, subDraw, movPerc);
      return PickMol(m, mk, bSrc, subDraw, boxDiv, mOff);
   }
