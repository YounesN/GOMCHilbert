...
class SubdividedArray
{
 public:
   ...
   { start[div] = first; start[div + 1] = first + len; }

   ~SubdividedArray(void) { Cleanup(); }
   void Cleanup(void) { delete[] start; start = NULL; }

   //returns index of the offset-th element of kind
   uint Index(const uint kind, const uint offset) const { return start[kind]+offset; }
