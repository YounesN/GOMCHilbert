...
class MoleculeLookup
{
 public:
   ...
   MoleculeLookup() : molLookup(NULL), boxAndKindStart(NULL) {}
     void TotalAtomsMols (uint * numByBox, uint * numByKindBox, uint * numAtomsByBox, MoleculeKind * kinds) const;// 
   ~MoleculeLookup()
   { delete[] molLookup; delete[] boxAndKindStart; }
...
class MoleculeLookup::box_iterator
{
   ...
   bool operator<= (const box_iterator& rhs) const { return !(rhs < *this); }
   bool operator>= (const box_iterator& rhs) const { return !(*this < rhs); }
   box_iterator& operator++ () { ++pIt; return *this; }
